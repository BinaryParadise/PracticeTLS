...
//        * generation qualifier (e.g., "Jr.", "3rd", or "IV").
        ...
        init?(asn1sequence : ASN1Sequence)
        {
            ...
            case .rsassa_pss:
                ...
                guard let hashAlgorithm = HashAlgorithm(oid: (((parameters[0] as? ASN1TaggedObject)?.object as? ASN1Sequence)?[0] as? ASN1ObjectIdentifier)?.oid ?? .sha1) else {
                    LogError("Error: unsupported hash algorithm \(parameters)")
                    return nil
                }
                ...
                guard let saltLengthInt = saltLength.intValue else {
                    LogError("Error: ridiculously long salt length: \(saltLength.value)")
                    return nil
                }
            ...
            default:
                LogError("Unsupported signature algorithm \(algorithmOID)")
                return nil
            }
    ...
    public struct Certificate
    {
        ...
        init?(derData : Data) {
            self.init(derData: derData.bytes)
        }
